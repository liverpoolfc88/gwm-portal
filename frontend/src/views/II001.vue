<template>
  <v-card class="ma-2 pa-3">
    <v-card-title>
      <span class="ml-5">GWM II001 files</span>
      <v-spacer></v-spacer>
        <v-btn v-if="$store.getters.checkPermission('ii001_file-send')" 
          class="ml-5 success" @click="filesend('II001')" :loading="loading1">
          send II001 files
      </v-btn>
    </v-card-title>
    <v-data-table
...
export default {
  ...
  methods: {
    ...
    getList() {
      this.loading = true;
      this.$axios.post(this.$store.state.backend_url + "api/ii001files" ,{
